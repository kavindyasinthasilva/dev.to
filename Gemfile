...
gem "activerecord-import", "~> 1.0", ">= 1.0.3" # Adds ability to bulk create activerecord objects
...
gem "aws-sdk-lambda", "~> 1.30", ">= 1.30.0" # Official AWS Ruby gem for AWS Lambda
...
gem "counter_culture", "~> 2.2", ">= 2.2.4" # counter_culture provides turbo-charged counter caches that are kept up-to-date
gem "dalli", "~> 3.2", ">= 3.2.3" # High performance memcached client for Ruby
gem "delayed_job_active_record", "~> 4.1", ">= 4.1.5" # ActiveRecord backend for Delayed::Job
gem "delayed_job_web", "~> 1.4", ">= 1.4.3" # Web interface for delayed_job
...
gem "feedjira", "~> 3.0", ">= 3.0.0" # A feed fetching and parsing library
gem "figaro", "~> 1.1" # Simple, Heroku-friendly Rails app configuration using ENV and a single YAML file
gem "fog-aws", "~> 3.5", ">= 3.5.2" # 'fog' gem to support Amazon Web Services
...
gem "google-api-client", "~> 0.33", ">= 0.33.1" # Client for accessing Google APIs
...
gem "jbuilder", "~> 2.9", ">= 2.9.1" # Create JSON structures via a Builder-style DSL
...
gem "octokit", "~> 4.14", ">= 4.14.0" # Simple wrapper for the GitHub API
...
gem "rails-observers", "~> 0.1", ">= 0.1.5" # Rails observer (removed from core in Rails 4.0)
gem "recaptcha", "~> 5.2", ">= 5.2.1", require: "recaptcha/rails" # Helpers for the reCAPTCHA API
gem "redcarpet", "~> 3.5", ">= 3.5.1" # A fast, safe and extensible Markdown to (X)HTML parser
gem "reverse_markdown", "~> 1.3", ">= 1.3.0" # Map simple html back into markdown
...
gem "sass-rails", "~> 6.0", ">= 6.0.0" # Sass adapter for the Rails asset pipeline
...
gem "skylight", "~> 4.1", ">= 4.1.2" # Skylight is a smart profiler for Rails, Sinatra, and other Ruby apps
...
gem "twilio-ruby", "~> 5.28", ">= 5.28.0" # The official library for communicating with the Twilio REST API
gem "twitter", "~> 6.2", ">= 6.2.0" # A Ruby interface to the Twitter API
...
group :development do
  ...
  gem "brakeman", "~> 4.7", ">= 4.7.1", require: false # Brakeman detects security vulnerabilities in Ruby on Rails applications via static analysis
  gem "bundler-audit", "~> 0.6" # bundler-audit provides patch-level verification for Bundled apps
  gem "derailed_benchmarks", "~> 1.4", ">= 1.4.1", require: false # A series of things you can use to benchmark a Rails or Ruby app
...
group :development, :test do
  ...
  gem "bullet", "~> 6.0", ">= 6.0.2" # help to kill N+1 queries and unused eager loading
  gem "capybara", "~> 3.29", ">= 3.29.0" # Capybara is an integration testing tool for rack based web applications
  ...
  gem "rspec-rails", "~> 3.9", ">= 3.9.0" # rspec-rails is a testing framework for Rails 3+
  ...
  gem "rubocop-rails", "~> 2.3", ">= 2.3.2", require: false # Automatic Rails code style checking tool
...
group :test do
  ...
  gem "launchy", "~> 2.4", ">= 2.4.3" # Launchy is helper class for launching cross-platform applications in a fire and forget manner.
  ...
  gem "webdrivers", "~> 4.1", ">= 4.1.2" # Run Selenium tests more easily with install and updates for all supported webdrivers
  gem "webmock", "~> 3.7", ">= 3.7.6" # WebMock allows stubbing HTTP requests and setting expectations on HTTP requests
  gem "zonebie", "~> 0.6.1" # Runs your tests in a random timezone
end

group :doc do
  gem "sdoc", "~> 1.0", ">= 1.0.0" # rdoc generator html with javascript search index
end
